...
ser = serial.Serial('/dev/cu.usbmodem12341', 250000, timeout=0)
...
while True:
    ...
    # Read EPROM
    if option == 1:
        name = input("Enter filename to dump EPROM content: ")
        ...
        while numBytes < romsize:
            while ser.inWaiting() == 0:
                print("Reading from EPROM. Current percentage:{:.2%}".format(numBytes/romsize), end='\r')
                time.sleep(0.1)
    ...
    # Just some info
    if option == 3:
        print("\nA more detailed write up about this project is available at www.dragaosemchama.com")
        print("This script goes together with an Arduino sketch, both are used to read and program")
    ...
    if option == 4:
        ...
        while numBytes < romsize:
            while ser.inWaiting() == 0:
                print("Reading from EPROM. Current percentage:{:.2%} Bytes:{} size:{}".format(numBytes/romsize, numBytes, romsize), end='\r')
                time.sleep(0.1)
            data = ser.read(1)
            numBytes = numBytes+1
            if ord(data) != 255:
    ...
    if option == 6:
        ...
        while numBytes < romsize:
            ...
            if EPROM_byte != file_byte:
                ...
                # I don't know how windows handle them
